...
<x-layouts.base>
 ...
 bagian atas halaman. --}}
            <nav class="glass flex h-14 items-center gap-5 rounded-full ps-5 pe-2.5">
                <a href="{{ url('/') }}" class="flex shrink-0 items-center gap-2.5">
                    <span class="flex size-[22px] items-center justify-center rounded-sm bg-accent text-xs font-bold text-accent-on">
                        {{ mb_substr(config('app.name'), 0, 1) }}
                    </span>
                    <span class="text-body font-semibold tracking-tight text-ink">{{ config('app.name') }}</span>
                </a>

                <div class="hidden flex-1 gap-1 md:flex">
                    <a href="#fitur" class="rounded-full px-3 py-1.5 text-base text-ink-secondary transition hover:bg-surface-inset hover:text-ink">Fitur</a>
                    <a href="#harga" class="rounded-full px-3 py-1.5 text-base text-ink-secondary transition hover:bg-surface-inset hover:text-ink">Harga</a>
                    @if ($docsEnabled)
                        <a href="{{ route('design-system.foundation') }}" class="rounded-full px-3 py-1.5 text-base text-ink-secondary transition hover:bg-surface-inset hover:text-ink">Design system</a>
                    @endif
                </div>

                <div class="ms-auto flex items-center gap-2 md:ms-0">
                    <button type="button" data-theme-toggle
 class="inline-flex size-9 items-center justify-center rounded-full text-ink-muted transition hover:bg-surface-inset hover:text-ink focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)]">
 ...
 class="flex h-9 items-center rounded-full bg-accent px-4 text-base font-semibold text-accent-on shadow-sm transition hover:bg-fill-2">
                            Buka dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="px-2 text-base text-ink-secondary transition hover:text-ink">Masuk</a>
 ...
 class="flex h-9 items-center rounded-full bg-accent px-4 text-base font-semibold text-accent-on shadow-sm transition hover:bg-fill-2">
            ...
            <div class="mx-auto max-w-[760px] pt-20 text-center">
                <span class="inline-flex items-center gap-2 rounded-full border border-line bg-surface-raised px-3.5 py-1.5 text-sm text-ink-secondary">
                    <span class="size-1.5 rounded-full bg-success"></span>
                    Laravel {{ Illuminate\Foundation\Application::VERSION }} · PHP {{ PHP_MAJOR_VERSION }}.{{ PHP_MINOR_VERSION }}
                </span>

                <h1 class="mt-6 text-4xl leading-[1.05] font-semibold tracking-[-0.035em] text-ink sm:text-6xl">
                    Hak akses yang berubah lewat panel, bukan lewat deploy.
                </h1>

                <p class="mx-auto mt-5 max-w-[56ch] text-lg text-ink-secondary sm:text-lg">
 ...
 class="flex h-12 items-center gap-2 rounded-md bg-accent px-6 text-body font-semibold text-accent-on shadow-md transition hover:bg-fill-2">
 ...
 class="flex h-12 items-center gap-2 rounded-md border-[0.5px] border-line bg-fill-3 px-5 text-body font-medium text-ink shadow-sm transition hover:bg-fill-2">
                ...
                <p class="mt-4 text-sm text-ink-muted">Tanpa pustaka UI pihak ketiga · Tanpa font dari CDN · Terang dan gelap setara</p>
            </div>
            ...
            <div class="glass mt-14 rounded-xl p-3.5">
                <div class="overflow-hidden rounded-lg border border-line bg-surface-raised">
                    <div class="flex h-9 items-center gap-2 border-b border-line bg-surface-sunken px-3.5">
                        ...
                        <span class="size-[9px] rounded-full bg-line-strong"></span>
                        <span class="size-[9px] rounded-full bg-line-strong"></span>
                        <span class="ms-3 font-mono text-xs text-ink-muted">{{ parse_url(config('app.url'), PHP_URL_HOST) ?: 'localhost' }}/admin/resources</span>
                    </div>
 ...
 seketika, tanpa satu baris kode pun berubah.
    ...
    {{-- ──────────────────────────────────────────────────────────── Fitur --}}
    <section id="fitur" class="mt-28 border-t-[0.5px] border-line bg-surface-raised px-6 py-28">
        <div class="mx-auto max-w-[1120px]">
            <h2 class="mx-auto max-w-[20ch] text-center text-4xl font-semibold text-ink sm:text-5xl">
            ...
            <div class="mt-14 grid gap-px overflow-hidden rounded-xl border-[0.5px] border-line bg-line sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($features as $feature)
                    <div class="bg-surface-raised p-6">
                        <x-ui.icon :name="$feature['icon']" class="size-5 text-accent" />
                        <h3 class="mt-3.5 text-lg font-semibold text-ink">{{ $feature['title'] }}</h3>
    ...
    {{-- ──────────────────────────────────────────────────────────── Harga --}}
    <section id="harga" class="border-t-[0.5px] border-line px-6 py-28">
        <div class="mx-auto max-w-[1120px]">
            <h2 class="mx-auto max-w-[16ch] text-center text-4xl font-semibold text-ink sm:text-5xl">Harga</h2>
            <p class="mx-auto mt-4 max-w-[52ch] text-center text-body text-ink-secondary">
                Sekali bayar, tanpa langganan. Angka di bawah ini isi contoh — ganti dengan harga Anda sendiri.
            </p>

            {{-- Hanya satu tingkat yang diberi penekanan. Tiga kartu yang
                 sama-sama berteriak membuat orang membandingkan hiasannya,
                 bukan isinya. --}}
            <div class="mt-14 grid gap-3 lg:grid-cols-3">
                @foreach ($plans as $plan)
                    <div @class([
                        'flex flex-col rounded-xl p-7',
                        'bg-surface-raised shadow-md ring-2 ring-accent' => $plan['featured'],
                        'border-[0.5px] border-line bg-surface-raised' => ! $plan['featured'],
                    ])>
                        ...
                        <div class="mt-5 flex items-baseline gap-2">
                            <span class="num text-4xl font-semibold text-ink">{{ $plan['price'] }}</span>
                            <span class="text-sm text-ink-muted">{{ $plan['note'] }}</span>
                        </div>
 ...
 block class="mt-6">
    ...
    {{-- ────────────────────────────────────────────────────────── Penutup --}}
    <section class="border-t-[0.5px] border-line bg-surface-raised px-6 py-28">
        <div class="mx-auto max-w-[1120px] text-center">
            <h2 class="mx-auto max-w-[24ch] text-3xl leading-tight font-semibold tracking-[-0.03em] text-ink sm:text-4xl">
    ...
    <footer class="border-t-[0.5px] border-line px-6 py-10">
        <div class="mx-auto flex max-w-[1120px] flex-wrap items-center justify-between gap-4 text-sm text-ink-muted">
            <span>{{ config('app.name') }} · boilerplate Laravel</span>
            <span class="text-sm">auth · rbac resource key · design system</span>
        </div>
    </footer>
</x-layouts.base>
