...
<div class="flex flex-wrap items-center gap-2.5">
 ...
 class="block h-[34px] w-[230px] max-w-full rounded-md border-[0.5px] border-line bg-fill-4 ps-9 pe-3 text-base text-ink outline-none transition placeholder:text-ink-muted focus:shadow-[var(--focus-ring)]">
        </div>
 ...
 dikirim. Chip mengirim dirinya sendiri lewat tautan. --}}
    ...
    @if ($table?->hasActiveFilters())
        <a href="{{ $table->resetUrl() }}" class="text-base text-ink-muted underline-offset-2 transition hover:text-ink hover:underline">
            Reset
        </a>
    @endif
    ...
    @isset($bulk)
        <div class="ms-auto flex items-center gap-2.5">
            <span class="text-sm text-ink-muted" x-show="selected.length" x-cloak
 x-text="selected.length + ' dipilih'"></span>

            <div class="flex items-center gap-2 transition-opacity duration-[--dur-fast]"
