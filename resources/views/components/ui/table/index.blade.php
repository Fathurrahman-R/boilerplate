...
<div class="overflow-hidden rounded-lg border-[0.5px] border-line bg-surface-raised shadow-sm"
    ...
    <table {{ $attributes->class('w-full text-left text-base text-ink-secondary') }}>
        <thead class="bg-fill-4">
            <tr>
                @if ($isSelectable)
 ...
 aria-label="Pilih semua baris"
                ...
                @else
                    @foreach ($headers as $column => $label)
                        ...
                        <th scope="col" class="px-4 py-2.5 text-xs font-semibold tracking-[0.05em] whitespace-nowrap text-ink-muted uppercase">
                            @if ($sortable)
