...
<div class="border-b border-line" x-data="{
    ...
    <button type="button" x-on:click="toggle()" :aria-expanded="expanded"
        ...
        <span class="flex shrink-0 transition-transform duration-[--dur-base]" :class="expanded && 'rotate-180'">
            <x-ui.icon name="chevron-down" class="size-4 text-ink-muted" />
        </span>
    </button>

    <div class="grid transition-[grid-template-rows] duration-[--dur-base] ease-[var(--ease-out-apple)]"
         :class="expanded ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'">
        <div class="overflow-hidden">
            <div class="pb-3.5 text-base text-ink-secondary">{{ $slot }}</div>
        </div>
    </div>
</div>
