...
<div x-data="{
 ...
 if (this.draft === '') this.tags.pop();
    ...
    <div class="flex min-h-control flex-wrap items-center gap-1.5 rounded-md border bg-fill-4 p-1.5 transition focus-within:border-accent focus-within:shadow-[var(--focus-ring)] {{ $invalid ? 'border-danger' : 'border-line' }}"
 x-on:click="$refs.draft.focus()">

        <template x-for="(tag, index) in tags" :key="tag">
            <span class="inline-flex items-center gap-1.5 rounded-full border-[0.5px] border-line bg-fill-3 py-1 ps-2.5 pe-1.5 text-[12.5px] text-ink shadow-sm">
