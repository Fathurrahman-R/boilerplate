...
<div class="relative">
    <div class="glass rounded-xl p-7">
        ...
        <p class="mt-3.5 text-lg leading-snug font-medium tracking-tight text-ink">{{ $quote }}</p>

        <div class="mt-5 flex items-center gap-2.5">
            <span class="flex size-9 shrink-0 items-center justify-center rounded-full border-[0.5px] border-line bg-surface-inset text-sm font-semibold text-ink-secondary">
                {{ $initials }}
            </span>
            <div>
                <div class="text-sm font-semibold text-ink">{{ $name }}</div>
                <div class="text-sm text-ink-muted">{{ $role }}</div>
            </div>
        </div>
    </div>
    ...
    @if ($stats !== [])
        <div class="relative mt-6 flex flex-wrap gap-7">
            @foreach ($stats as $stat)
                <div>
                    <div class="text-2xl font-semibold text-ink">{{ $stat['value'] }}</div>
