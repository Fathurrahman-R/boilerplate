...
<x-layouts.base :title="$title ? $title.' — Design system' : 'Design system'">
    <header class="glass sticky top-0 z-40 rounded-none border-x-0 border-t-0">
        <div class="mx-auto flex h-[62px] max-w-[1240px] items-center gap-4 px-6">
            <a href="{{ url('/') }}" class="flex shrink-0 items-center gap-2.5">
                <span class="flex size-[26px] items-center justify-center rounded-sm bg-accent text-sm font-bold text-accent-on">
                    {{ mb_substr(config('app.name'), 0, 1) }}
                </span>
                <span class="text-base font-semibold tracking-tight text-ink">{{ config('app.name') }}</span>
                <span class="hidden rounded-full border-[0.5px] border-line px-2 py-px font-mono text-2xs text-ink-muted sm:inline">
 design system
                </span>
            </a>
            ...
            <nav class="ms-4 hidden gap-1 md:flex">
                @foreach ($pages as $route => $label)
                    <a href="{{ route('design-system.'.$route) }}"
                       @class([
                           'rounded-sm px-2.5 py-1.5 text-base transition',
                           'bg-surface-inset font-semibold text-ink' => request()->routeIs('design-system.'.$route),
                           'text-ink-secondary hover:bg-fill-3 hover:text-ink' => ! request()->routeIs('design-system.'.$route),
                       ])>
                        {{ $label }}
                    </a>
                @endforeach
 ...
 class="hidden text-base text-ink-muted transition hover:text-ink sm:inline">
 ...
 class="inline-flex size-9 items-center justify-center rounded-md text-ink-secondary transition hover:bg-fill-3 hover:text-ink focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)]">
    ...
    <div class="mx-auto grid max-w-[1240px] items-start gap-14 px-6 lg:grid-cols-[196px_minmax(0,1fr)]">
        <nav class="sticky top-[62px] hidden flex-col gap-px py-10 text-base lg:flex">
            @foreach ($nav as $key => $value)
                @if (is_array($value))
                    <span class="eyebrow px-2.5 pt-4 pb-2 first:pt-0">{{ $key }}</span>

                    @foreach ($value as $anchor => $label)
                        <a href="{{ $anchor }}" class="rounded-sm px-2.5 py-1.5 text-ink-secondary transition hover:bg-fill-3 hover:text-ink">{{ $label }}</a>
                    @endforeach
                @else
                    <a href="{{ $key }}" class="rounded-sm px-2.5 py-1.5 text-ink-secondary transition hover:bg-fill-3 hover:text-ink">{{ $value }}</a>
                @endif
            @endforeach
        </nav>
        ...
        <main class="min-w-0 py-10 pb-28">
            ...
            <footer class="mt-24 flex flex-wrap items-center justify-between gap-4 border-t border-line pt-7 text-sm text-ink-muted">
