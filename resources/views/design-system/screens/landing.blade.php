...
<x-docs.screen :screen="$screen" :meta="$meta" url="nusatagih.id">
        ...
        <div class="relative mx-auto max-w-[1000px] px-7 pt-5">
            <nav class="glass flex h-14 items-center gap-5 rounded-full ps-5 pe-2.5">
                <span class="flex shrink-0 items-center gap-2.5">
                    <span class="flex size-[22px] items-center justify-center rounded-sm bg-accent text-xs font-bold text-accent-on">N</span>
                    <span class="text-body font-semibold tracking-tight text-ink">Nusatagih</span>
                </span>

                <div class="hidden flex-1 gap-1 md:flex">
                    @foreach (['Produk', 'Harga', 'Dokumentasi', 'Pelanggan'] as $link)
                        <span class="rounded-full px-3 py-1.5 text-base text-ink-secondary">{{ $link }}</span>
                    @endforeach
                </div>

                <span class="ms-auto px-2 text-base text-ink-secondary md:ms-0">Masuk</span>
                <span class="flex h-9 items-center rounded-full bg-accent px-[17px] text-base font-semibold text-accent-on shadow-sm">
                    Coba gratis
                </span>
            </nav>
            ...
            <div class="mx-auto max-w-[720px] pt-[68px] text-center">
                ...
                <h1 class="mt-5 text-[38px] leading-[1.05] font-semibold tracking-[-0.035em] text-ink sm:text-[50px]">
                    Penagihan yang berhenti memakan hari Senin Anda.
                </h1>

                <p class="mx-auto mt-4 max-w-[54ch] text-lg text-ink-secondary">
 ...
 menyalin nomor dari satu tab ke tab lain.
                ...
                <div class="mt-7 flex flex-wrap justify-center gap-3">
                    <span class="flex h-12 items-center gap-2 rounded-md bg-accent px-6 text-body font-semibold text-accent-on shadow-md">
                        Mulai 14 hari gratis
                        <x-ui.icon name="arrow-up-right" class="size-[17px]" />
                    </span>
                    <span class="flex h-12 items-center gap-2 rounded-md border-[0.5px] border-line bg-fill-3 px-5 text-body font-medium text-ink shadow-sm">
                        <x-ui.icon name="play" class="size-4" />
                        Lihat demo
                    </span>
                </div>

                <p class="mt-3.5 text-sm text-ink-muted">Tanpa kartu kredit · Migrasi data dibantu</p>
            </div>
                ...
                <div class="overflow-hidden rounded-lg border border-line bg-surface-raised">
                    <div class="grid gap-px bg-line sm:grid-cols-3">
                        <div class="bg-surface-raised p-5">
                            ...
                            <div class="mt-1.5 text-2xl font-semibold tabular-nums text-ink">1.284</div>
                            <div class="mt-1 flex items-center gap-1.5 text-sm text-success">
                                <x-ui.icon name="trending-up" class="size-3.5" />+68 bulan ini
                            </div>
                        </div>
                        <div class="bg-surface-raised p-5">
                            <span class="eyebrow">Dibayar tepat waktu</span>
                            <div class="mt-1.5 text-2xl font-semibold tabular-nums text-ink">92,6%</div>
                            <div class="mt-1 text-sm text-ink-muted">Sebelumnya 71,4%</div>
                        </div>
                        <div class="bg-surface-raised p-5">
                            <span class="eyebrow">Jam kerja hemat</span>
                            <div class="mt-1.5 text-2xl font-semibold tabular-nums text-ink">31/bln</div>
                            <div class="mt-1 text-sm text-ink-muted">Rata-rata pelanggan</div>
                        </div>
                    </div>
            ...
            <div class="mt-10 flex flex-wrap items-center justify-center gap-9 opacity-70">
                @foreach (['Nusantara', 'Sinar Abadi', 'Delta', 'Aksara', 'Kopi Rakyat'] as $logo)
                    <span class="text-lg font-semibold text-ink-muted">{{ $logo }}</span>
                @endforeach
            </div>
        </div>
    </div>
    ...
    <div class="border-t border-line bg-surface-raised px-7 py-16">
        <div class="mx-auto max-w-[1000px]">
            <h2 class="max-w-[20ch] text-3xl leading-tight font-semibold tracking-[-0.03em] text-ink">
    ...
    <div class="border-t border-line px-7 py-16">
        <div class="mx-auto max-w-[1000px]">
            <h2 class="text-3xl leading-tight font-semibold tracking-[-0.03em] text-ink">Harga</h2>
            <p class="mt-2.5 max-w-[54ch] text-body text-ink-secondary">Bayar per bulan, berhenti kapan saja.</p>
                ...
                @foreach ($plans as $plan)
                    <div @class([
                        ...
                        <div class="mt-5 flex items-baseline gap-1.5">
                            <span class="text-3xl font-semibold tabular-nums text-ink">{{ $plan['price'] }}</span>
    ...
    <div class="border-t border-line bg-surface-raised px-7 py-16 text-center">
        <h2 class="mx-auto max-w-[24ch] text-3xl leading-tight font-semibold tracking-[-0.03em] text-ink">
