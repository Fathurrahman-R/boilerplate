...
<x-docs.screen :screen="$screen" :meta="$meta" url="tools.contoh.id/persetujuan">
    <div class="flex min-h-[720px] flex-col">
        ...
        <div class="flex items-center gap-3 border-b border-line bg-surface-sunken px-5 py-3">
            <span class="flex size-6 items-center justify-center rounded-sm bg-accent text-sm font-bold text-accent-on">O</span>
            <span class="text-body font-semibold tracking-tight text-ink">Operasi Internal</span>

            <div class="ms-4 flex items-center gap-1.5 text-sm text-ink-muted">
                <span>Keuangan</span>
            ...
            <span class="flex h-8 items-center gap-2 rounded-md border border-line bg-surface-raised px-2.5 text-sm text-ink-muted ">
                <x-ui.icon name="search" class="size-3.5" />
                Cari
                <kbd class="rounded-[4px] border border-line px-1.5 font-mono text-2xs">⌘K</kbd>
            </span>
        </div>
        ...
        <div class="flex min-h-0 flex-1 flex-col xl:flex-row">
            ...
            <div class="min-w-0 flex-1">
                ...
                <table class="w-full text-left text-base text-ink-secondary">
                    ...
                    <tbody>
                        @foreach ($queue as $item)
                            <tr @class([
                                ...
                                <td class="num px-5 py-2.5 text-sm">{{ $item['ref'] }}</td>
                                ...
                                <td class="num px-5 py-2.5 text-right text-sm">{{ $item['amount'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>

                <div class="flex items-center justify-between gap-3 border-t border-line px-5 py-3 text-sm text-ink-muted">
                    ...
                    <div class="flex gap-1.5">
                        ...
                        <span class="flex size-8 items-center justify-center rounded-sm bg-accent text-sm font-semibold text-accent-on">1</span>
                        <span class="flex size-8 items-center justify-center rounded-sm border border-line bg-surface">
            ...
            <aside class="w-full shrink-0 border-t border-line bg-surface-sunken xl:w-[400px] xl:border-t-0 xl:border-s">
                <div class="flex items-start justify-between gap-3 border-b border-line px-5 py-4">
                    <div>
                        <span class="num text-sm text-ink-muted">REQ-4821</span>
                        <h2 class="mt-0.5 text-lg font-semibold text-ink">Pencairan dana operasional</h2>
                    </div>
                ...
                <div class="space-y-5 px-5 py-5">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="num mt-1 text-body text-ink">Rp 18.400.000</div>
                        </div>
                        <div>
                            <span class="eyebrow">Anggaran</span>
                            <div class="mt-1 text-base text-ink">Operasional Q3</div>
                        </div>
                        <div>
                            <span class="eyebrow">Pengaju</span>
                            <div class="mt-1 text-base text-ink">Maya S.</div>
                        </div>
                        <div>
                            <span class="eyebrow">Diajukan</span>
                            <div class="num mt-1 text-base text-ink">{{ now()->subDays(2)->format('d/m/Y') }}</div>
                        </div>
                    </div>

                    <div>
                        <span class="eyebrow">Keterangan</span>
                        <p class="mt-1.5 text-base text-ink-secondary">
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex flex-col gap-1.5">
                            @foreach (['kontrak-vendor-2026.pdf', 'rincian-biaya.xlsx'] as $file)
                                <span class="flex items-center gap-2.5 rounded-md border border-line bg-surface-raised px-3 py-2 text-sm text-ink">
                        ...
                        <div class="mt-2.5 space-y-3">
                            @foreach ($timeline as $entry)
                                <div class="flex gap-3">
                                    <span class="num shrink-0 text-xs text-ink-muted">{{ $entry['time'] }}</span>
